...
| ADR-009 | Prefijo global y rutas relativas en controllers | Cerrado | 2026-03-23 | El versionado de rutas se define exclusivamente en `main.ts`; queda prohibido repetir `api/v1` en `@Controller(...)` |
...
### ADR-009 — Prefijo global y rutas relativas
Define la convención oficial de rutas HTTP. El prefijo global del API
se declara exclusivamente en `src/main.ts` mediante `app.setGlobalPrefix('api/v1')`.
Los controllers deben declarar rutas relativas al recurso, por ejemplo
`@Controller('auth')`, `@Controller('users')` o `@Controller('cases')`.

Queda prohibido repetir `api/v1` dentro de `@Controller(...)`.
Cualquier excepción deberá justificarse mediante un nuevo ADR.
Esta decisión obliga a alinear controllers existentes, contrato API,
smoke tests y documentación operativa.
