...
#
# FLUJO DE REVISIÓN:
#   1. POST /review (crea revisión vigente con resultado)
#   2. POST /transition (valida revisión compatible y transiciona)
#
...
# =============================================================================
# FASE 6: DEVOLUCIÓN (flujo: POST /review + POST /transition)
# =============================================================================
echo ""
echo "--- FASE 6: DEVOLUCIÓN (review + transition) ---"

# Paso 1: Crear revisión con resultado='devuelto'
if [ -n "$CASE_ID" ]; then
  RESP=$(curl -sS -w "\n%{http_code}" -X POST "$BASE_URL/cases/$CASE_ID/review" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"resultado":"devuelto","observaciones":"Revisar elemento subjetivo del dolo."}')
  HTTP=$(echo "$RESP" | tail -1)
  BODY=$(echo "$RESP" | sed '$d')
  if [ "$HTTP" = "201" ]; then
    log_pass "10. POST /review (resultado=devuelto) → 201"
  else
    log_fail "10. POST /review → $HTTP"
    log_body "$BODY"
  fi
else
  log_fail "10. POST /review → Sin CASE_ID"
fi

# Paso 2: Transicionar a devuelto
if [ -n "$CASE_ID" ]; then
  RESP=$(curl -sS -w "\n%{http_code}" -X POST "$BASE_URL/cases/$CASE_ID/transition" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
  ...
  if [ "$HTTP" = "200" ] || [ "$HTTP" = "201" ]; then
    ESTADO=$(echo "$BODY" | jq -r '.estado_actual')
    [ "$ESTADO" = "devuelto" ] && log_pass "11. Transición pendiente_revision→devuelto → $HTTP" || log_fail "11. Estado: $ESTADO"
  else
    log_fail "11. Transición devuelto → $HTTP"
    log_body "$BODY"
  fi
else
  log_fail "11. Transición → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ "$REV_COUNT" -ge 1 ] && [ "$REV_DEVUELTO" -ge 1 ]; then
    log_pass "12. GET /review → $REV_COUNT revisiones (1+ devuelto) ✓"
  else
    log_fail "12. GET /review → $REV_COUNT revisiones, $REV_DEVUELTO devuelto"
  fi
else
  log_fail "12. GET /review → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ -n "$OBS" ] && [ "$RESULTADO" = "devuelto" ]; then
    log_pass "13. GET /review/feedback → observaciones + resultado=devuelto ✓"
  else
    log_fail "13. GET /review/feedback → OBS='$OBS', RESULTADO='$RESULTADO'"
  fi
else
  log_fail "13. GET /review/feedback → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ "$HTTP" = "200" ] || [ "$HTTP" = "201" ]; then
    ESTADO=$(echo "$BODY" | jq -r '.estado_actual')
    [ "$ESTADO" = "en_analisis" ] && log_pass "14. Transición devuelto→en_analisis → $HTTP" || log_fail "14. Estado: $ESTADO"
  else
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ "$HTTP" = "200" ] || [ "$HTTP" = "201" ]; then
    ESTADO=$(echo "$BODY" | jq -r '.estado_actual')
    [ "$ESTADO" = "pendiente_revision" ] && log_pass "15. Transición en_analisis→pendiente_revision (2da) → $HTTP" || log_fail "15. Estado: $ESTADO"
  else
...
# Crear revisión con resultado='aprobado'
if [ -n "$CASE_ID" ]; then
  RESP=$(curl -sS -w "\n%{http_code}" -X POST "$BASE_URL/cases/$CASE_ID/review" \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"resultado":"aprobado","observaciones":"Caso aprobado. Analisis corregido satisfactoriamente."}')
  HTTP=$(echo "$RESP" | tail -1)
  BODY=$(echo "$RESP" | sed '$d')
  if [ "$HTTP" = "201" ]; then
    log_pass "16. POST /review (resultado=aprobado) → 201"
  else
    log_fail "16. POST /review → $HTTP"
    log_body "$BODY"
  fi
else
  log_fail "16. POST /review → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ "$HTTP" = "200" ] || [ "$HTTP" = "201" ]; then
    ESTADO=$(echo "$BODY" | jq -r '.estado_actual')
    [ "$ESTADO" = "aprobado_supervisor" ] && log_pass "17. Transición pendiente_revision→aprobado_supervisor → $HTTP" || log_fail "17. Estado: $ESTADO"
  else
    log_fail "17. Transición → $HTTP"
    log_body "$BODY"
  fi
else
  log_fail "17. Transición → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  if [ "$REV_COUNT" -ge 2 ] && [ "$REV_DEVUELTO" -ge 1 ] && [ "$REV_APROBADO" -ge 1 ]; then
    log_pass "18. GET /review → $REV_COUNT revisiones (devuelto=$REV_DEVUELTO, aprobado=$REV_APROBADO) ✓"
  else
    log_fail "18. GET /review → Total=$REV_COUNT, Devuelto=$REV_DEVUELTO, Aprobado=$REV_APROBADO"
  fi
else
  log_fail "18. GET /review → Sin CASE_ID"
fi
...
if [ -n "$CASE_ID" ]; then
  ...
  [ "$ESTADO" = "aprobado_supervisor" ] && log_pass "19. GET /cases/:id (estado=aprobado_supervisor) → OK" || log_fail "19. Estado final: $ESTADO"
else
  log_fail "19. GET /cases/:id → Sin CASE_ID"
fi
...
if [ "$PASSED" -eq 19 ] && [ "$FAILED" -eq 0 ]; then
  ...
  echo -e "  ${GREEN}✓ Flujo review→transition operativo${NC}"
...
elif [ "$FAILED" -eq 0 ] && [ $((PASSED + SKIPPED)) -eq 19 ]; then
  echo -e "  ${CYAN}○ Cierre con skips controlados (PASSED=$PASSED, SKIPPED=$SKIPPED)${NC}"
  echo ""
  echo -e "${CYAN}E5-05 CIERRE PARCIAL POSIBLE${NC}"
else
