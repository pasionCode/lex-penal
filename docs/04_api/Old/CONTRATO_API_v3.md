...
Campos principales: `etapa_procesal`, `regimen_procesal`, `proximo_acto`,
`fecha_proximo_acto`, `responsable_acto`.
...
de hechos. Cada hecho incluye: `descripcion`, `fuente`, `relevancia`,
...
`descripcion`, `tipo`, `hecho_id`, `hecho_descripcion_libre`,
`licitud`, `legalidad`, `suficiencia`, `observaciones`.
...
Campos principales: `linea_principal`, `lineas_subsidiarias`,
`posicion_acusacion`, `posicion_pena`, `observaciones`.
...
Campos principales: `situacion_libertad`, `opciones_presentadas`,
`decision_cliente`, `fecha_explicacion`, `observaciones`.
  ...
  "bloques": [
    {
      "id": "uuid",
      "orden": 1,
      "etiqueta": "Bloque 1. [Etiqueta según U008]",
      "completado": false,
      "items": [
        {
          "id": "uuid",
          "texto": "[Criterio según U008]",
          "marcado": false,
          "critico": true
        }
      ]
...
Campos principales: `sintesis_juridica`, `posicion_defensiva`,
`recomendacion_estrategica`, `proximas_actuaciones`, `observaciones_finales`.
...
**Valores válidos de `herramienta`**: `ficha_basica`, `matriz_hechos`,
`matriz_probatoria`, `matriz_riesgos`, `estrategia_defensa`,
`explicacion_cliente`, `checklist_calidad`, `conclusion_operativa`.
...
**Parámetros opcionales**
- `tipo` — filtra por tipo de evento: `transicion_estado`, `ia_query`, `revision`, `documento`.
- `page`, `per_page` — paginación.
